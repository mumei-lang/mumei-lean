...
`known_witness_used = false`. There are twenty live generated theorem paths in
...
`take_point` (struct projection), `join_all_last_result` (task group all),
`race_two_replicas` (task group any)); see
