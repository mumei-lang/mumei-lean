...
- Live generated theorem coverage now includes three paths: `std/math/abs.mm::abs_saturating`, `std/math/patterns.mm::bounded_mul_with_overflow_check`, and `std/crypto/primitives.mm::constant_time_eq_flag`. The bounded-multiplication path uses complete `body_expr` lowering plus mathlib-backed nonlinear arithmetic splitting; the crypto path lowers Mumei braced conditional body semantics for a deterministic 0/1 witness. Both upgrade to `lean_verified` with `known_witness_used = false` after Lake succeeds.
